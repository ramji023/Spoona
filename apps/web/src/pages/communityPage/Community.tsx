...
export default function Community() {
  ...
  const removeUserMutation = useMutation({
    ...
    onError: (err: Error | any) => {
      ...
      if (err.request) {
        ...
        setFailureMsg(err.response.data?.message || "Opeation failed");
      } else {
  ...
  // call react query to fetch the community data
  const query = useCommunity(communityId);
  const { data, isLoading, error } = useMinLoader({ query, loadingTime: 1000 });

  if (!communityId) return <Err />;
  ...
  if (data) {
    // calculate that if user is joined or not
    const joined =
